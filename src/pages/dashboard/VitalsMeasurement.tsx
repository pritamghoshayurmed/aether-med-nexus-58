...
import { Activity, Heart, Wind, Camera, X, CheckCircle, AlertCircle, Loader2 } from "lucide-react";
...
const VitalsMeasurement = () => {
  ...
  const startRecording = useCallback(() => {
    ...
    captureIntervalRef.current = window.setInterval(() => {
      ...
      if (frame) {
        ...
        // Check signal quality every 3 seconds
    ...
    toast({
      ...
      description: `Hold your finger steady for ${RECORDING_DURATION} seconds. Analysis will start automatically.`,
    });
  ...
  const stopRecording = useCallback((frames?: string[]) => {
    ...
    if (framesToProcess.length >= TARGET_FPS * 5) { // At least 5 seconds
  ...
  const checkSignalQuality = async (frames: string[]) => {
    try {
      const response = await fetch(`${PPG_SERVICE_URL}/test/signal-quality`, {
        ...
        body: JSON.stringify({
          frames: frames.slice(0, 10) // Send first 10 frames for quick check
        }),
      });
      ...
      // Don't show error to user during quality check - it's optional
    }
  };
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({
          ...
          description: `Heart Rate: ${result.vitals.heart_rate} BPM`,
        });
      } else {
      ...
      if (err.message === 'Failed to fetch' || err.name === 'TypeError') {
        setError(`PPG Analysis Service is not running. Please start the service at ${PPG_SERVICE_URL}`);
        toast({
  ...
  const saveVitalsToDatabase = async (vitals: any) => {
    try {
      ...
      // Insert vitals - Only save measurable PPG vitals
      ...
      if (insertError) {
        ...
        toast({
          title: "Save Error",
          description: "Could not save vitals to database. Please try again.",
          variant: "destructive",
        });
      } else {
        ...
        toast({
          ...
          description: `Vitals saved to your medical records (Patient ID: ${patientData.id})`,
        });
      }
    } catch (err) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-bold mb-2">
            <Activity className="inline-block mr-2 h-8 w-8 text-primary" />
            PPG Vitals Measurement
          </h1>
          <p className="text-muted-foreground">
            Measure your heart rate and vitals using your camera
          </p>
        </div>
        ...
        <MedicalCard variant="glass" className="mb-6">
          <MedicalCardHeader>
            <MedicalCardTitle>How to Measure with PPG</MedicalCardTitle>
          </MedicalCardHeader>
          <MedicalCardContent>
            <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
              <p className="text-sm font-medium text-blue-900 dark:text-blue-100">
                📊 Measurable Vitals via PPG:
              </p>
              <ul className="list-disc list-inside text-sm text-blue-800 dark:text-blue-200 mt-2 ml-2">
            ...
            <ol className="list-decimal list-inside space-y-2 text-sm">
              ...
              <li>Place your index finger gently over the rear camera lens</li>
              <li>Ensure the camera flash is on (automatically enabled)</li>
              <li>Hold your finger steady and click "Start Recording"</li>
              <li>Keep your finger still for 30 seconds</li>
              <li>Wait for analysis to complete</li>
            </ol>
            <Alert className="mt-4">
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>
                <strong>Tip:</strong> For best results, sit still and apply gentle pressure. Ensure finger completely covers the camera and flash.
              </AlertDescription>
            </Alert>
          </MedicalCardContent>
        </MedicalCard>
        ...
        <MedicalCard variant="glass">
          <MedicalCardHeader>
            <MedicalCardTitle>About PPG Measurement</MedicalCardTitle>
          </MedicalCardHeader>
          <MedicalCardContent className="text-sm text-muted-foreground">
            <p className="mb-2">
              <strong>Photoplethysmography (PPG)</strong> uses your camera and flash to detect blood flow changes in your finger. 
              This technology can measure:
            </p>
            <ul className="list-disc list-inside space-y-1 ml-2">
              <li><strong>Heart Rate (HR)</strong> - Number of heartbeats per minute [Accurate]</li>
              <li><strong>Heart Rate Variability (HRV)</strong> - Variation in time between heartbeats [Accurate]</li>
              <li><strong>Respiratory Rate (RR)</strong> - Number of breaths per minute [Estimated from signal modulation]</li>
            </ul>
            <div className="mt-4 p-3 bg-amber-50 dark:bg-amber-900/20 rounded border border-amber-200 dark:border-amber-800">
              <p className="text-xs font-semibold text-amber-900 dark:text-amber-100 mb-1">⚠️ What PPG CANNOT Measure:</p>
              <ul className="list-disc list-inside text-xs text-amber-800 dark:text-amber-200 ml-2">
                <li>Body Temperature</li>
                <li>Blood Pressure</li>
                <li>Blood Oxygen (SpO2) - Requires dual-wavelength sensor</li>
              </ul>
            </div>
            <p className="mt-3 text-xs">
              <strong>Note:</strong> This is a research-grade algorithm based on validated studies. 
              It should not replace professional medical devices for clinical decisions.
            </p>
          </MedicalCardContent>
        </MedicalCard>
      </div>
